...
- [改进] Web/桌面端统一采用克制的金融信息终端视觉语言：以语义化明暗主题变量重塑排版、表格、表单、导航、提示与图表周边样式，并修复桌面侧栏 rail 模式内容未水平居中的问题；页面布局、路由、交互与业务功能保持不变。
- [改进] 首页报告收敛为更克制的金融信息视图：情绪仪表盘取消渐变、光晕与数值动画，并以悲观红、中性琥珀黄、乐观绿复用策略区状态色；策略与历史列表改用细线和状态色表达，保持原有信息结构与交互不变。
